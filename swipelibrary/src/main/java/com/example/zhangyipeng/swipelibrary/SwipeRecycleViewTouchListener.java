...
package com.example.zhangyipeng.swipelibrary;
 ...
 */
public class SwipeRecycleViewTouchListener implements View.OnTouchListener {
    ...
    public SwipeRecycleViewTouchListener(SwipeRecycleView swipeRecycleView, int swipeFrontView, int swipeBackView) {
        ...
        configShortAnimationTime = swipeRecycleView.getContext().getResources().getInteger(android.R.integer.config_shortAnimTime);
    ...
    private void setBackView(View backView) {
        ...
        backView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                swipeRecycleView.onClickBackView(downPosition);
            }
        });
    }
    ...
    public void resetItems() {
        if (swipeRecycleView.getAdapter() != null) {
            int count = swipeRecycleView.getAdapter().getItemCount();
    ...
    protected void openAnimate(int position) {
        final View child = swipeRecycleView.getChildAt(position - mLayoutManager.findFirstVisibleItemPosition()).findViewById(swipeFrontView);
    ...
    protected void closeAnimate(int position) {
        if (swipeRecycleView != null) {
            ...
            final View childContainer = swipeRecycleView.getChildAt(position - firstVisibleChildPosition);
    ...
    private void swapChoiceState(int position) {
        ...
        if (lastCount == 0 && count == 1) {
            swipeRecycleView.onChoiceStarted();
            closeOpenedItems();
            setActionsTo(SwipeListView.SWIPE_ACTION_CHOICE);
        }
        if (lastCount == 1 && count == 0) {
            swipeRecycleView.onChoiceEnded();
            returnOldActions();
        }
//        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
//            swipeRecycleView.setItemChecked(position, !lastChecked);
//        }
        swipeRecycleView.onChoiceChanged(position, !lastChecked);
    ...
    protected void unselectedChoiceStates() {
        ...
        for (int i = 0; i < checked.size(); i++) {
            if (checked.get(i) && i >= start && i <= end) {
                reloadChoiceStateInView(swipeRecycleView.getChildAt(i - start).findViewById(swipeFrontView), i);
            }
            checked.set(i, false);
        }
        swipeRecycleView.onChoiceEnded();
    ...
    protected int dismiss(int position) {
        ...
        int start = mLayoutManager.findFirstVisibleItemPosition();
        int end = mLayoutManager.findLastVisibleItemPosition();
        View view = swipeRecycleView.getChildAt(position - start);
    ...
    protected void reloadSwipeStateInView(View frontView, int position) {
        if (!opened.get(position)) {
            ...
            if (openedRight.get(position)) {
                setTranslationX(frontView, swipeRecycleView.getWidth());
            } else {
                setTranslationX(frontView, -swipeRecycleView.getWidth());
            }
        }
        ...
        animate(view)
                ...
                .setListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        swipeRecycleView.resetScrolling();
                        resetCell();
                    }
                });
    }
                ...
                .setListener(new AnimatorListenerAdapter() {
                    ...
                    public void onAnimationEnd(Animator animation) {
                        ...
                        if (swap) {
                            ...
                            if (onlyOneOpenedWhenSwipe) {
                                if (aux) {
                                    swipeRecycleView.onOpened(position, swapRight);
                                } else {
                                    swipeRecycleView.onClosed(position, openedRight.get(position));
                                }
                            } else {
...
//                    if (onFirstItemList)
//                        isFirstItem = true;
//                        swipeRecycleView.onFirstListItem();
//                    }
//                }
//                if (isLastItem) {
...
//                    if (onLastItemList) {
//                        isLastItem = true;
//                        swipeRecycleView.onLastListItem();
//                    }
//                }
            }
        };
    ...
    void closeOpenedItems() {
        if (opened != null) {
            ...
            for (int i = start; i <= end; i++) {
                if (opened.get(i)) {
                    closeAnimate(swipeRecycleView.getChildAt(i - start).findViewById(swipeFrontView), i);
                }
            }
        }
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        if (viewWidth < 2) {
            viewWidth = swipeRecycleView.getWidth();
        }
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            case MotionEvent.ACTION_DOWN: {
                ...
                int childCount = swipeRecycleView.getChildCount();
                int[] listViewCoords = new int[2];
                swipeRecycleView.getLocationOnScreen(listViewCoords);
                int x = (int) motionEvent.getRawX() - listViewCoords[0];
                int y = (int) motionEvent.getRawY() - listViewCoords[1];
                View child;
                for (int i = 0; i < childCount; i++) {
                    child = swipeRecycleView.getChildAt(i);
                    child.getHitRect(rect);

                    int childPosition = swipeRecycleView.getChildPosition(child);

                    // dont allow swiping if this is on the header or footer or IGNORE_ITEM_VIEW_TYPE or enabled is false on the adapter
                    //boolean allowSwipe = swipeRecycleView.getAdapter().isEnabled(childPosition) && swipeRecycleView.getAdapter().getItemViewType(childPosition) >= 0;
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                int changeSwipeMode = swipeRecycleView.changeSwipeMode(downPosition);
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListView.SWIPE_ACTION_NONE && velocityY < velocityX) {
                    ...
                    if (opened.get(downPosition)) {
                        swipeRecycleView.onStartClose(downPosition, swipingRight);
                        ...
                        swipeRecycleView.onStartOpen(downPosition, swipeCurrentAction, swipingRight);
                    }
                    swipeRecycleView.requestDisallowInterceptTouchEvent(true);
                    MotionEvent cancelEvent = MotionEvent.obtain(motionEvent);
                    cancelEvent.setAction(MotionEvent.ACTION_CANCEL |
                            (MotionEventCompat.getActionIndex(motionEvent) << MotionEventCompat.ACTION_POINTER_INDEX_SHIFT));
                    swipeRecycleView.onTouchEvent(cancelEvent);
                    if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_CHOICE) {
    ...
    public void move(float deltaX) {
        swipeRecycleView.onMove(downPosition, deltaX);
...
class PendingDismissData implements Comparable<PendingDismissData> {
    ...
    private void removePendingDismisses(int originalHeight) {
        ...
        swipeRecycleView.onDismiss(dismissPositions);
